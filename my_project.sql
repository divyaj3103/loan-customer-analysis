...
create table Loan_status (loan_id varchar(45), customer_id varchar(50),
	loan_amount varchar(45),
	loan_term_amount int, cibil_score int,
	primary key(loan_id));

	create table remarks_update(loan_id varchar(35), loan_amount varchar(60),
	cibil_score int, cibil_score_status varchar(100),primary key(loan_id));
	...
	create table loan_update select loan_status.loan_id,  loan_status.customer_id,loan_status.loan_amount,
	loan_status.loan_term_amount,loan_status.cibil_score,
	remarks_update.cibil_score_status
	from loan_status inner join remarks_update on loan_status.loan_id = 
	remarks_update.loan_id;
    ...
    create table loan_details select loan_update.loan_id,loan_update.customer_id,loan_update.loan_amount,loan_update.loan_term_amount,loan_update.cibil_score,
    loan_update.cibil_score_status,country_State.customer_name,country_State.region_id,country_State.postal_code,country_State.segment,
    country_State.state,customer_det.gender,customer_det.age,customer_det.married,customer_det.education,customer_det.self_employed,
    customer_income.applicantIncome,customer_income.coapplicantIncome,customer_income.property_area,customer_income.loan_status,
    region_info.region from loan_update inner join country_State on loan_update.loan_id = country_State.loan_id 
    inner join customer_det on  country_State.loan_id  = customer_det.loan_id 
    inner join customer_income on customer_det.loan_id = customer_income.loan_id
    inner join region_info on customer_det.region_id = region_info.region_id;
    
    select * from loan_details;
    
    select * from country_state right join region_info on country_state.region_id =  region_info.region_id;
    
    create table loan_update_details select *,
    case
    when applicantIncome >15000 then "A_grade"
    when applicantIncome >9000 then "B_grade"
    when applicantIncome >5000 then "Middle_class"
    else "Low_class"
    ...
    delete from loan_update_details where loan_amount = "loan still processing";
    alter table loan_update_details modify loan_amount int;
    
    -- calculation for monthly interest and annual interest 
   create table loan_det_amount select*, round((loan_amount * Interest_rate)/100,2) as monthly_interest, 
    round(((loan_amount * Interest_rate)/100 * 12),2) as annual_interest from loan_update_details;
    
    select * from loan_det_amount;
    
    
-- doing update in table customer_Det
...
-- calculation for customer id ,applicant income and loan amount 
     select state, count(customer_id) as customer_count, sum(loan_amount) as loan_count , sum(applicantIncome)as total from loan_update_details 
     group by  state order by state;
     
-- sorting last 4 digit name in customer name
     select customer_name,right(customer_name,4) as Last_name from customer_det where customer_name like "%____";
     
     
     delimiter @@
     create procedure MySqlProject()
     begin
     
     -- inner join (joining 5 tables)
        select * from loan_details;
        
	-- selecting certeria with grade based on applicantIncome
	    select * from loan_update_details;
        
	-- right join
        select * from country_state right join region_info on country_state.region_id =  region_info.region_id;
        
	-- calculating loan amount in monthly and annually
		select * from loan_det_amount;
        
	-- calculaing total from states
       select state, count(customer_id) as customer_count, sum(loan_amount) as loan_count , sum(applicantIncome)as total from loan_update_details 
        group by  state order by state;
        
	-- seperating last 4 digit in name 
		select customer_name,right(customer_name,4) as Last_name from customer_det where customer_name like "%____";
     end @@
     delimiter ;
